...
- **Crash safety** — CRC-verified entries, atomic hint file generation (`write → fdatasync → rename`), and append-only data files as the primary durable store. Hint files are an index, not the record: one that fails its CRC is rebuilt from its data file at recovery rather than dropped, so a damaged index costs the time to rebuild it and not the keys behind it. A data file that comes back *shorter* than the hint indexing it fails no CRC at all — recovery checks each index entry against the file it indexes rather than opening with the keys past the tear silently missing, and under `fail_recovery_on_crc_errors = false` it recovers every key below the tear instead of losing the whole file. On unrecoverable write-path failures (e.g. isolation rotation fails), the engine enters a degraded state: reads remain available, all writes throw `DbDegraded`, and the service calls `resume()` to recover without a restart.
...
struct Options {
    ...
    // or one that is shorter than the hint indexing it.
    // When true (default): DB::open throws. When false: the DB opens with a
    // warning on stderr for each — a file that cannot be rescanned is skipped,
    // and a short one keeps every key below the tear.
