...
- **Vacuum** — vacuum process to reclaim unused space from overwritten or deleted keys; query performance does not degrade as the database grows. Delete tombstones are reclaimed too: when vacuum compacts a file, it drops each tombstone that no longer hides an older value in another file. Which ones qualify is decided when the database is opened, so a tombstone written since the last open stays until the next one. Like the overwritten values vacuum removes, a dropped tombstone is gone from `changes_since`, so the stream is refused below the highest sequence vacuum has dropped (`min_resumable_sequence()`, throwing `DbInvalidSequence`): a follower that far behind re-bootstraps from a manifest instead of missing a delete.
...
public:
    ...
    // The lowest from_sequence changes_since accepts: vacuum has dropped no
    // entry above it. Never decreases; survives a restart.
