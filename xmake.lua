...
target("unordered_view_tests")
    set_kind("binary")
    set_default(false)
    add_files("benchmarks/unordered_view_test.cpp", "bytecaskdb/*.cppm")
...
target("map_bench")
    set_kind("binary")
    set_default(false)
    add_files("benchmarks/map_bench.cpp", "bytecaskdb/*.cppm")
    add_cxflags("-Wno-global-constructors")
    add_packages("benchmark", "crc32c", "zstd")
    add_defines("BYTECASK_TESTING")
    on_config(function(t)
        add_native_syslinks(t)
        apply_sanitizer(t)
...
target("memory_profile")
    ...
    add_packages("crc32c", "zstd", "jemalloc")
    add_defines("BYTECASK_TESTING")
    on_config(function(t)
        add_native_syslinks(t)
        apply_sanitizer(t)
...
target("fuzz_data_entry")
    set_kind("binary")
    set_default(false)
    add_files("tests/fuzz/fuzz_data_entry.cpp", "bytecaskdb/*.cppm")
    add_includedirs("bytecaskdb")
    add_packages("crc32c", "zstd")
    add_defines("BYTECASK_TESTING")
    on_config(function(t)
...
target("fuzz_hint_entry")
    set_kind("binary")
    set_default(false)
    add_files("tests/fuzz/fuzz_hint_entry.cpp", "bytecaskdb/*.cppm")
    add_includedirs("bytecaskdb")
    add_packages("crc32c", "zstd")
    add_defines("BYTECASK_TESTING")
    on_config(function(t)
...
target("gen_fuzz_corpus")
    set_kind("binary")
    set_default(false)
    add_files("tests/fuzz/gen_corpus.cpp", "bytecaskdb/*.cppm")
    add_includedirs("bytecaskdb")
    add_packages("crc32c", "zstd")
    on_config(function(t)
        add_native_syslinks(t)
...
-- Common WASM source files and the crc32c and zstd dependencies, both built
...
local function add_wasm_sources()
    ...
    -- A system include, as the native targets get it from the package:
    -- zstd.h does not build clean under -Weverything.
    add_sysincludedirs(path.join(wasm_zstd, "include"))
    add_linkdirs(path.join(wasm_crc32c, "lib"), path.join(wasm_zstd, "lib"))
    add_links("crc32c", "zstd")
end
