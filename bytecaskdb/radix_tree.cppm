...
// The reclaimer is shared with the B+ tree: same chain contract, same
// per-node interval free rule, one implementation. These traits are all it
// needs of a node — its version tag, its children, how to destroy it, and
// the accounting hook the memory tests read.
template <typename V> struct RadixChainTraits {
  using Node = bytecask::Node<V>;
  static auto tag(const Node *n) noexcept -> std::uint64_t {
    return n->edit_tag();
  }
  template <typename F> static void for_each_child(Node *n, F &&f) {
    typename Node::ChildCursor cursor;
  ...
  static void account_retired(std::int64_t delta) noexcept {
    detail::account_retired<V>(delta);
  }
};

template <typename V, typename Pred>
void free_subtree_if(Node<V> *root, Pred is_garbage) {
  free_node_subtree_if<RadixChainTraits<V>>(root, is_garbage);
}
...
private:
  ...
  static auto chain() -> VersionChain<RadixChainTraits<V>> & {
    return VersionChain<RadixChainTraits<V>>::instance();
  }
