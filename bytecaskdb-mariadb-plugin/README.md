...
| `bytecaskdb_recovery_threads` | 4 | 1 – 64 | no | Threads replaying hint files at startup. Each builds a partial key directory and the results are merged, so more threads also mean more peak memory during recovery: under a tight memory limit, fewer can be faster. |
