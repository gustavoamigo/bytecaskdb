...
| Background compaction | Vacuum is caller-driven. The plugin runs a background thread (launched at init, stopped at deinit) that calls `DB::vacuum()` in a loop. Fragmentation threshold and the busy/idle pause between passes are the `bytecaskdb_vacuum_*` system variables (defaults 0.5, 500 ms, 30 s), re-read on every pass. |
