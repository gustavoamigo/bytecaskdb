...
TEST_CASE("serialize_entry produces correct byte layout", "[serialize]") {
  ...
  // Key size at offset 8
  CHECK(read_le<std::uint16_t>(buf, 8) ==
        static_cast<std::uint16_t>(key.size()));

  // Value size at offset 10
  CHECK(read_le<std::uint32_t>(buf, 10) ==
        static_cast<std::uint32_t>(value.size()));

  // Flags at offset 14 should be 0 (active)
  CHECK(static_cast<std::uint8_t>(buf[14]) == 0U);
