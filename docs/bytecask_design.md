...
The `sanitizers` CI job matrixes `bytecask_tests` over AddressSanitizer, ThreadSanitizer, and MemorySanitizer (`--sanitizer=address|thread|memory`). MSan additionally requires an MSan-instrumented `libc++`/`libc++abi` — the system `libcxx-devel` package used by the other two is not instrumented and produces false positives if linked into an MSan binary — built by `scripts/build_msan_libcxx.sh` from the matching `llvm-project` release branch and cached across CI runs (`actions/cache`, keyed on the script's contents). Because the prebuilt `catch2` package is compiled against the system's default libstdc++ and can't link against a `-stdlib=libc++` binary, the MSan configuration of `bytecask_tests` compiles Catch2 from source instead, from the vendored amalgamated distribution in `third_party/catch2_amalgamated/`; every other configuration keeps using the normal `catch2` package. See `docs/correctness_validation.md` for the full rationale and known limitations.
...
- **Recovery via `resume()`**: the degraded flag is in-memory only. The service calls `resume()` to attempt in-process recovery without a restart. `resume()` runs a universal recovery process:
  ...
  2. Scans the active file using `CommittedEntryIterator` to find the last valid committed offset. Orphaned `BulkBegin` batches are excluded — if a `BulkBegin` has no matching `BulkEnd`, `committed_offset` is reset to before the batch start. A corrupt entry ends the scan where a CRC failure throws out of the iterator, so `valid_offset` is recorded as the scan advances rather than after it completes: the entries collected for replay and the offset the file is truncated to must describe the same prefix, or step 3 replays key directory entries addressing bytes step 4 removes (`validate_state_consistency` then rejects the resumed state and the engine stays degraded, with the committed prefix already truncated away).
...
Sequence numbers are unique per logical write. When two `KeyDirEntry` values claim the same key, the one with the higher sequence wins. Two entries with the same sequence are the same logical write met twice, which is not corruption: vacuum copies a file's live entries into a compacted file under their original sequences and only then unlinks the source, so a kill inside that window leaves both files on disk (see *Crash safety* under *Vacuum*). `kde_newer` is therefore a total order on `(sequence, file_id, file_offset)` — the higher file id wins, then the higher offset — rather than a check that throws. What it still rejects is a same-key, same-sequence pair whose value sizes differ: a copy is byte for byte its source, so that is two writes under one sequence, which no engine path produces. A throw from any recovery phase now leaves `DB::open` as an exception: `recovery_load_ranged`'s `parallel_for` catches on each thread and rethrows on the caller's after joining, where before an exception in the range merge was `std::terminate`. Every recovery path (one worker's sorted build, the pairwise merge, the ranged merge) uses this order, so merge results are independent of worker count, completion order or which worker each file went to. Recovery also assigns file ids in name order rather than directory order (`recovery_prepare_files` sorts the `.data` paths), which makes the ids a function of the directory's contents and makes the tie-break prefer the compacted copy, whose stem is the later one.
...
After the rename there is a second window: the compacted file is in place, its hint is being generated by a full scan, the state is committed, and only then is the source unlinked. A kill anywhere in there — seconds, for a 64 MiB file; a cgroup OOM kill under a write-heavy load found it — leaves the source and its compacted copy both on disk, holding the same entries under the same sequences. Recovery treats that as what it is: the same write seen twice. `kde_newer` picks one copy per key by a total order (the higher file id, which name-ordered id assignment makes the compacted file), so the source comes back with no live bytes and the next vacuum unlinks it through `vacuum_remove_file`, with no rewrite. Until that vacuum, D18 (sequence-disjoint files) does not hold for that pair, and `changes_since` yields the pair's entries twice; `ingest` is idempotent on sequence, so a follower is not corrupted, but the duplicate yield is tracked as follow-up work (#129). Before this the comparator threw on the pair and the database would not open.
