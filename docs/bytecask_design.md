...
**WriteGroup**: a writer that arrives while no leader is active becomes the leader, takes every queued slot (its own included) as one batch, and executes them all under one lock hold. N concurrent sync writes share a single `fdatasync` instead of paying N separate calls. This is the dominant performance win — fdatasync is ~1–2 ms; pwritev is ~microseconds. The leader keeps taking batches while slots are queued, but for at most `kMaxLeaderBatches` (8) in a row; then it returns to its caller and passes leadership to the head of the queue by setting that slot's `lead` flag and broadcasting. Every batch end is one broadcast on a shared condition variable — members of the finished batch return, the rest re-check and sleep — and the leader checks the queue only *after* re-acquiring the lock behind that broadcast, by which time members it just released have often resubmitted and join the next batch. (Two alternatives measured worse: a wake per slot, 2× slower at 32 writers; and deciding before the broadcast, which retires the leader whenever the queue is momentarily empty so the first resubmitter starts a batch of one.) The bound exists because the leader's own slot is done after its first batch: every further batch it runs is time its caller waits. An unbounded leader, under closed-loop writers — clients that resubmit as soon as their commit returns — never sees an empty queue, and one client thread became a permanent writer whose commit did not return for the length of the run (sysbench `oltp_write_only`, 8 threads: max latency 13 s, all fdatasyncs from one TID). The bound is not 1 because a hand-off costs a thread wake-up before the next batch can start, while a leader that stays hot starts the next batch the moment the previous one ends; rotating every batch measured 30–40% lower `PutMT/Sync` throughput at 16–32 writers on 4 cores. Batch formation is unchanged either way: slots accumulate during the running batch's `fdatasync`.
...
- **`WritableMmapDataFile`**: Pre-allocates and zero-fills the file to the rotation threshold (`WritableFileOps::preallocate`, see *Preallocation with written extents* below), then maps it with `mmap(PROT_READ, MAP_SHARED)`. Reads within the mapped region are zero-syscall memcpy from the mmap region. Reads beyond the mmap region (rare: overflow past pre-allocated size) fall back to `pread`. Writes go through `pwritev` at a tracked `offset_`. MAP_SHARED is required so that `pwritev` writes through the fd are visible to mmap readers.
- **`WritablePosixDataFile`**: Pure `pread`-based reads, `pwritev` writes. Same preallocation and zero-fill as the mmap variant, except under Emscripten (MEMFS), where the file simply grows. This is the default, and the fallback for platforms without mmap support.
...
- **`shrink_to_fit()`**: `ftruncate` to `size()` plus `fdatasync`. Called once, when the file is sealed, to give the preallocated tail back. Unlike `truncate()` it never touches a mapping, so it is safe while readers hold snapshots of the file — every published offset lies below `size()`.
...
- **Preallocation with written extents**: Every writable data file is grown to `max_file_bytes` at creation and zero-filled (`WritableFileOps::preallocate`: `fallocate` on Linux, `ftruncate` elsewhere, then 1 MiB `pwrite`s of zeros and one `fsync`). Allocation alone is not enough: `fallocate` reserves *unwritten* extents, and the first write into each block converts one — a journaled metadata change that the next `fdatasync` must wait for. On ext4 (`data=ordered`) that turns a commit into data write → journal write → journal commit → flush, roughly 2× the cost of a data-only flush; measured on an Azure SSD, 2.9 ms per `fdatasync` versus 1.6 ms once the extents are written, and MariaDB sysbench `oltp_write_only` went from 1.3k to 2.3k tps. With the extents written, every later `fdatasync` is a pure data flush with no journal traffic — the same profile as a preallocated redo log. The cost is one sequential write of `max_file_bytes` per rotation, paid by the group-commit leader under the write lock: ~0.5 s for 64 MiB on the same disk, once per 64 MiB of appended data. That stall was judged cheaper than the machinery to pre-create the next file off the write path; revisit if a workload shows it matters. Under Emscripten the file simply grows: MEMFS has no journal to avoid and a preallocated tail would be resident memory.
- **Sealed files are exact-size**: the zero tail is not a format concern — every `scan()` stops at a zero header (`sequence == 0`), which no real entry can carry — but `file_size` seeds `total_bytes` at recovery, and a 64 MiB tail on a half-full file would make vacuum see it as mostly garbage. So the tail is given back whenever a file is sealed: `rotate_active_file` and vacuum's staging copy call `shrink_to_fit()` before the file is reopened read-only, `~DB` calls it on the active file, and `recovery_prepare_files` truncates a hint-less data file (the active file of a crashed engine) to the committed end that `flush_hints_for` returns from its scan. Files sealed by earlier builds in mmap mode keep their preallocated size and an existing hint; recovery seeds an inflated `total_bytes` for them once, and vacuum rewrites them early.
