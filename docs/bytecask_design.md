...
Internal (non-leaf) nodes are tiered by fanout: `Node4` embeds up to 4 children inline in a single allocation (no separate child-store object or buffers), and `InternalNode` — the "Large" tier, unbounded fanout via a heap-allocated `ChildStore` — takes over once fanout exceeds 4. A `Node4` promotes to `InternalNode` on its 5th child; an `InternalNode` demotes back to `Node4` when its count drops to 4 or fewer. This tiering targets the fanout distribution actually seen in practice: chain-compression routing nodes (created whenever a key exceeds `CompactPrefix`'s 7-byte inline cap) have exactly 1 child, and prefix splits start at 2 — the low end where a single-allocation `Node4` beats `InternalNode`'s separate `ChildStore` object and its two backing buffers. Measured on genuinely random key shapes (`sha256_hex`, `uuidv4_binary` — the ones with the most chain-compression nodes), this cuts net heap bytes by ~22–25% at 100K keys with no change on shapes with little chain compression (`zipfian`) and flat-to-marginally-better on the two officially tracked `map_bench` shapes (`uniform`, `prefixed`), which use dense sequential-suffix keys whose internal nodes mostly land in the Large tier regardless. `child_count`, `child_at`, `find_child`, and the mutation helpers funnel through checked accessors (`as_node4()`, `internal_node()`) that dispatch on `node_type()` and return safe defaults for leaf nodes, instead of repeating flag checks plus open-coded downcasts at each call site. `Node::insert_child` / `Node::remove_child` are the only two places that know about tier promotion/demotion — every other call site just sees "insert a child, get back the (possibly reallocated) node."
