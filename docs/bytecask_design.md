...
- The tree reads keys through a `KeyReader` over the `KeyDirCtx`: the whole entry, CRC-checked (unverified when `ReadOptions::verify_checksums` is off). A record the current batch has placed but not written is answered from `TransientEngineState::pending_`, filled as each put is applied and discarded with the transient.
...
- Recovery still builds the B+ tree from the hint files, then bulk-loads the blind tree from it in key order: correct, but peak memory at open is the B+ tree's until recovery can build the blind tree from the hint files directly (step 4 of the design).
