...
- **Barrier operations** — rotation, `create_manifest`, `resume`, `vacuum_commit`, `set_mode`, `ingest` — call `quiesce()` under `write_mu_`: take the role, flush and publish the head on the calling thread, and hold the role (RAII `FlushRole`) while they run the pre-pipeline code on the published state. `FlushRole`'s destructor resets `head_` to `state_`. Every publication happens under the role, so a flush that is landing can never overwrite a barrier's or a failure path's state.
- **The head's `durable_seq` lags.** The head chain does not learn about flushes, so only `execute_slots` may derive a state from `head_`; everything else derives from `state_` after `quiesce()`. `flush_pending` uses `max(head, published)` for a NoSync-only publish and `next_seq - 1` after an fdatasync.
