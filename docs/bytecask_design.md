...
A rebuild is only ever as complete as the data file it scans, and a data file that is **short** rather than corrupt fails no CRC anywhere: `scan_committed` stops cleanly at the first entry that does not frame, so a torn tail, a partial restore, or a copy that lost its last extent yields a shorter hint, recovery succeeds, and every key past the tear is simply absent.

When the hint is readable it is the record of what the file should hold. A hint entry names its record's `file_offset`, and its `value_size` sizes that record exactly for all three hint entry types — a `RangeDel` carries its end key as the value — so an entry ending past the data file's size names bytes the file no longer has. `ShortFileGuard` tests that for every entry as recovery scans it, inside the one pass both paths already make, before the entry is counted towards the file's sequence bounds or applied to the key directory.

A data file is append-only, so offsets rise with sequence and those entries are precisely its lost tail. Dropping them is what a rescan of the short file would produce: an older surviving write of a key wins, and a range tombstone whose bytes are gone was never committed as far as any reader can tell. Lenient recovery does that and keeps every key below the tear — which also keeps the file's bytes live, so the next vacuum has no reason to reclaim it. Rejecting the whole file instead would recreate the loss described above, where a file contributing no `live_bytes` is unlinked. Strict recovery throws, the outcome an unindexable file already gets. Because dropping an entry leaves the run ascending, the B+ tree path's bulk loader is fed the same way whether or not a file lost its tail.

This is the release-build enforcement of an invariant the engine otherwise only asserts in test builds: `validate_state_consistency` rejects a key directory entry that ends past its file's committed extent, but that walk is compiled out of a release build, and reaching it means recovery has already built the bad state rather than reporting which file was short.

The residual is the case where the hint is unreadable **and** the data file is short. The rebuild then produces a shorter hint that every entry passes trivially, and a sealed data file carries no external record of its own length to check it against, so nothing detects it without a format change. Sequence gaps are not a substitute — contiguity is not required, so a missing run of sequences cannot be told from a legitimate gap.
