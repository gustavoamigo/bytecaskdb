...
**Sealed file — `ReadOnlyBufferPoolDataFile`**: When `Options::io_backend` is `IoBackend::BufferPool`, sealed files are served from a bounded frame cache owned by the DB (`BufferPool`, `bytecaskdb/buffer_pool.cppm`). Reads are lock-free — an open-addressed probe keyed by `(cache_id, frame_index)`, then a seqlock-guarded copy out of the frame. The cache id is allocated by the pool when a file is opened, not the engine's `file_id`: the id only has to be unique, and the engine mints `file_id` at points the file factory does not reach. Fills and eviction (CLOCK) take one mutex, released across the device read. Values are always copied into the caller's buffer, never handed back as a span into a frame: frames are reused memory, so a span into one would dangle the moment it was evicted. `Options::buffer_pool.capacity_bytes` is the total footprint — frames, per-frame metadata and the index table all come out of it — and `DB::open` rejects a pool smaller than `2 x max_file_bytes`. See [`buffer_pool_design.md`](buffer_pool_design.md).
