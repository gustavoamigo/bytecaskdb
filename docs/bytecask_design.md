...
A rebuild is only ever as complete as the data file it scans, and a data file that is **short** rather than corrupt fails no CRC anywhere: `scan_committed` stops cleanly at the first entry that does not frame, so a torn tail, a partial restore, or a copy that lost its last extent yields a shorter hint, recovery succeeds, and every key past the tear is simply absent — the same permanence as a skipped file, since those bytes are then garbage for the next vacuum to rewrite.

When the hint is readable it is the record of what the file should hold. A hint entry names its record's `file_offset`, and its `value_size` sizes that record exactly for all three hint entry types — a `RangeDel` carries its end key as the value — so the end of the furthest entry is the extent the index depends on. `check_data_file_covers_hint` compares that extent against the data file's size once per file, immediately after `open_hint_or_rebuild` and before the file contributes anything, and throws on a shortfall. Both recovery paths call it inside the same per-file `try` that already applies `fail_recovery_on_crc_errors`, so the outcome is the one an unindexable file already gets: thrown on under strict, skipped with a warning under lenient. The check is one walk over a buffer the trailer CRC has just read, with no I/O of its own, and it runs on the recovery worker rather than serially at `recovery_prepare_files`.
