...
Internal (non-leaf) nodes are tiered by fanout: `Node4` embeds up to 4 children inline in a single allocation, `Node16` embeds up to 16 (same sorted-array-plus-linear-scan shape as `Node4`, just a bigger array), `Node48` embeds up to 48 (same sorted array, plus a 256-byte `child_index_` mapping a transition byte directly to its slot so `find_child` is O(1) instead of a scan), and `InternalNode` — the "Large" tier, unbounded fanout via a heap-allocated `ChildStore` — takes over once fanout exceeds 48. A `Node4` promotes to `Node16` on its 5th child, `Node16` promotes to `Node48` on its 17th, and `Node48` promotes to `InternalNode` on its 49th; `Node16`↔`Node4` and `InternalNode`↔`Node48` demote symmetrically at the same boundary, but `Node48`↔`Node16` uses proportional hysteresis (demotes at ≤12, not ≤16, matching DuckDB's ART) so a node fluctuating near the 16/17 boundary doesn't thrash. This tiering targets the fanout distribution actually seen in practice: chain-compression routing nodes (created whenever a key exceeds `CompactPrefix`'s 7-byte inline cap) have exactly 1 child and prefix splits start at 2 — where `Node4` wins — while `Node16` picks up the next band, which turns out to matter most for dense, sequential-suffix keys (the officially tracked `map_bench` shapes) rather than the genuinely random ones. Measured at 100K keys: `Node4` alone cut net heap bytes by ~22–25% on random shapes (`sha256_hex`, `uuidv4_binary`) with `uniform`/`prefixed` flat; adding `Node16` on top then cut `uniform` by ~11% and `prefixed` by ~19%, with the random shapes staying flat (their branching is dominated by the 1-child routing nodes `Node4` already covers); adding `Node48` on top of that measured flat on every shape tested — never worse, but the 17–48 fanout band turned out to be numerically thin everywhere, including the random shapes (see `docs/persistent_radix_tree_design.md` §7.9). `child_count`, `child_at`, `find_child`, and the mutation helpers funnel through checked accessors (`as_node4()`, `as_node16()`, `as_node48()`, `internal_node()`) that dispatch on `node_type()` and return safe defaults for leaf nodes, instead of repeating flag checks plus open-coded downcasts at each call site. `Node::insert_child` / `Node::remove_child` are the only two places that know about tier promotion/demotion — every other call site just sees "insert a child, get back the (possibly reallocated) node." A `Node256` tier built directly on top of `Node4` (skipping the 5–48 range) was tried and set aside after it caused an unexplained ~65–70% regression in tree iteration; building `Node48` surfaced and fixed the likely root cause of that class of bug — `Node::release()`'s tail-release loop was value-initializing a fixed-capacity stack array on every single node release regardless of that node's actual tier, a cost that scales with the array's static size rather than the work being done — see `docs/persistent_radix_tree_design.md` §7.8–§7.9 for the investigation and the plan to revisit `Node256` with the fix in hand.
