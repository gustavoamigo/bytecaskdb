...
Internal (non-leaf) nodes are tiered by fanout: `Node4` embeds up to 4 children inline in a single allocation, `Node16` embeds up to 16 (same sorted-array-plus-linear-scan shape as `Node4`, just a bigger array), and `InternalNode` — the "Large" tier, unbounded fanout via a heap-allocated `ChildStore` — takes over once fanout exceeds 16. A `Node4` promotes to `Node16` on its 5th child, a `Node16` promotes to `InternalNode` on its 17th, and both demote symmetrically when a removal drops the count back below the lower tier's capacity. This tiering targets the fanout distribution actually seen in practice: chain-compression routing nodes (created whenever a key exceeds `CompactPrefix`'s 7-byte inline cap) have exactly 1 child and prefix splits start at 2 — where `Node4` wins — while `Node16` picks up the next band, which turns out to matter most for dense, sequential-suffix keys (the officially tracked `map_bench` shapes) rather than the genuinely random ones. Measured at 100K keys: `Node4` alone cut net heap bytes by ~22–25% on random shapes (`sha256_hex`, `uuidv4_binary`) with `uniform`/`prefixed` flat; adding `Node16` on top then cut `uniform` by ~11% and `prefixed` by ~19%, with the random shapes staying flat (their branching is dominated by the 1-child routing nodes `Node4` already covers). `child_count`, `child_at`, `find_child`, and the mutation helpers funnel through checked accessors (`as_node4()`, `as_node16()`, `internal_node()`) that dispatch on `node_type()` and return safe defaults for leaf nodes, instead of repeating flag checks plus open-coded downcasts at each call site. `Node::insert_child` / `Node::remove_child` are the only two places that know about tier promotion/demotion — every other call site just sees "insert a child, get back the (possibly reallocated) node." A `Node256` tier built directly on top of `Node4` (skipping the 5–16 range) was tried and set aside after it caused an unexplained ~65–70% regression in tree iteration that four independent, disassembly-verified fix attempts could not resolve; see `docs/persistent_radix_tree_design.md` §7.8 for the investigation and the plan to revisit it once `Node48` exists.
