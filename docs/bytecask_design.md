...
**Sealed file — `ReadOnlyBufferPoolDataFile`**: When `Options::io_backend` is `IoBackend::BufferPool`, sealed files are served from a bounded frame cache owned by the DB (`BufferPool`, `bytecaskdb/buffer_pool.cppm`). Reads are lock-free — an open-addressed probe keyed by `(file_id, frame_index)`, then a seqlock-guarded copy out of the frame under relaxed atomic word accesses — the frame is being written concurrently, so plain `memcpy` there would be a data race. Vacuum reserves its destination `file_id` before opening the compacted file (`TransientEngineState::reserve_file_id`), because the file must carry its id from construction. Frames are filled with `O_DIRECT` through a second descriptor, so the pool — not the page cache — is the memory that holds sealed-file data; a filesystem that refuses falls back to buffered fills for that file, counted in `pool_direct_io_fallbacks`. Scans and oversize reads use the buffered descriptor. Fills and eviction (CLOCK) take one mutex, released across the device read. Values are always copied into the caller's buffer, never handed back as a span into a frame: frames are reused memory, so a span into one would dangle the moment it was evicted. `Options::buffer_pool.capacity_bytes` is the total footprint — frames, per-frame metadata and the index table all come out of it — and `DB::open` rejects a pool smaller than `2 x max_file_bytes`. See [`buffer_pool_design.md`](buffer_pool_design.md).
