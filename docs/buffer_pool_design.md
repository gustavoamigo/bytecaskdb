...
- **`insert(key, frame, bytes)`** takes a dead frame — claimed by eviction, or fresh and named by no slot — finds the first empty slot on the key's probe chain, marks it odd, stores frame and key, writes the frame's bytes, marks it even, and releases the pin word last, so a reader that pins it then finds the slot as it is.
...
The seqlock writer stores the odd version and then a release fence, so the changes that follow cannot become visible before the odd version does; the even version is a release store. The frame's bytes are published by the release store that clears the pin word, which the reader's acquiring `fetch_add` synchronises with — so a reader whose pin succeeds sees the finished bytes.
...
`mmap`'s own Range-50 moved 1.50 → 1.67 µs: `EntryIterator` now goes through `lend_entry`, and the lease reset on advance plus the `verify` branch inside it are ~20 instructions an entry that the mapping does not need. A base-class default for `lend_entry` cost a second virtual call on top (1.75 µs) and was replaced by per-back-end implementations.
