...
On macOS there is no `O_DIRECT`; the direct descriptor is opened normally and given `F_NOCACHE`, the analogue named above, which bypasses the buffer cache with no alignment requirement. There is no `posix_fadvise` either, so the page-cache releases below are no-ops there — the residency they release on Linux is never built up under `F_NOCACHE`. Where neither exists, every file takes the buffered fallback and is counted. The first `O_DIRECT` commit shipped none of this and broke the macOS wheel build; the guards follow the file's existing `#ifndef __APPLE__` convention.
...
**The table above is a warm page cache, and it made the `O_DIRECT` rows look 5–8x worse than they are.** Under `direct_io=1` a pool miss is a device read, and this VM's disk answers in ~50 µs; the `pread`, `mmap` and buffered-pool rows were reading the same bytes from a page cache the earlier arms had already warmed. So the first version of this section compared a pool paying device latency against baselines paying none and called the gap the pool's cost. The comparison that means something starts every back-end cold: `--cold` fsyncs and `POSIX_FADV_DONTNEED`s every data file before each run (no root needed — the pages are clean), and each run is on its own thread because the engine's thread-local read snapshot outlives the DB and kept an `mmap` run's mapping — and so its pages — alive into the next run. `cached_bytes` is `mincore` over the data files after the run; `rss_bytes` is the process.

| Back-end (cold) | direct | ratio | ops/s | p50 | p99 | p99.9 | hit ratio | page cache after | RSS after |
|---|:-:|---:|---:|---:|---:|---:|---:|---:|---:|
| pread | — | — | 294 K | 645 ns | 53.0 µs | 106 µs | — | 32.4 MB | 16.9 MB |
| mmap | — | — | 214 K | 341 ns | 57.6 µs | 104 µs | — | 27.4 MB | 44.5 MB |
| buffer pool | 0 | 0.25 | 275 K | 446 ns | 57.8 µs | 108 µs | 0.807 | 32.3 MB | 26.3 MB |
| buffer pool | 0 | 1.00 | 264 K | 517 ns | 64.4 µs | 148 µs | 0.928 | 32.4 MB | 50.8 MB |
| buffer pool | 1 | 0.25 | 94 K | 567 ns | 63.6 µs | 99 µs | 0.807 | **1.0 MB** | 26.9 MB |
| buffer pool | 1 | 1.00 | 225 K | 464 ns | 55.0 µs | 82 µs | 0.928 | **1.0 MB** | 51.3 MB |

Three things this settles. **The tail is the disk, for everyone.** p99 is 53–64 µs on every row: it is one device read, and once the page cache is cold every back-end pays it on a first touch. `O_DIRECT` does not make the tail worse; the warm table only made it look that way. **The pool's hit path is fine.** p50 under the pool (446–567 ns) sits between `mmap` (341 ns) and `pread` (645 ns), which is where §9 said it would. **Page-cache residency is the thing `direct_io` bounds, and it bounds it completely.** After a `pread`, `mmap` or buffered-pool run the whole 32 MB dataset is in the page cache — on this machine there was nothing to lose by that, on the machine §1 describes it is the key directory's memory. After a direct run the page cache holds 1 MB: the active file, which §6 keeps buffered on purpose. That column is the pool's deliverable, and it is the only column where the pool is not somewhere in the middle.

The throughput gap is real and has one cause: **readahead.** `pread` gets to 294 K/s cold because the kernel pulls the dataset in 128 KiB at a time — about 250 device reads for 31 MiB — while a direct-I/O miss is one 4 KiB frame, so at ratio 0.25 the pool takes ~11.5 k device reads where `pread` takes a few hundred, and 11.5 k × 50 µs is the 94 K/s. That arithmetic also says the gap closes as the working set fits (225 K/s at 1.00, 4.3 k compulsory misses) and would reverse the moment the dataset outgrows the page cache, where readahead becomes the thing thrashing the key directory. It is also the case for §10's "prefetch neighbouring frames on a miss" and for batched fills (`preadv2`, or a larger fill unit for sequential misses), both of which this row now has a number to be measured against. Neither is in this tree.

What this benchmark still cannot show is the pool's *benefit* on a wall clock: a dataset larger than RAM, or a cgroup limit, where the page cache is the thing evicting the key directory. Neither is set up here. On this evidence the pool holds `cached_bytes` at the active file, costs ~25 % of `pread`'s cold throughput at a ratio where the working set fits and ~3x where it does not, and has the same tail as everything else — which is an honest description of Phase 2 on a machine with spare RAM.
...
The get latencies in that table are **not** pool hit latencies, and an earlier version of this paragraph wrongly said they showed an unexplained ~1.7 µs active-file hit. Isolated (`pool_bench --hit-probe`: 200 k gets of one sealed key and one active key, no writes interleaved), a pool hit on the active file is **247 ns** against **217 ns** sealed — 30 ns apart, zero misses, zero retries. What the arm measures is a get immediately after a put, and with `staleness_tolerance = 0` that get refreshes its read snapshot and releases the old one, which every back-end pays. Why the pool's version of that transition is ~0.5 µs slower than `pread`'s is still open, but it is a question about the get-after-write path, not about the pool's hit.
