...
**Non-goals.** Making a dataset that already fits in RAM faster: on a resident hit the pool is within ~80 ns of `mmap` and cannot beat it (§8). Where memory is not constrained, leave it off.
...
```
Arena        one allocation of 4 KiB frames, holding atomic 64-bit words
...
Eviction     CLOCK over the index, test-then-set reference bit
...
**Frames hold atomic words, not plain bytes.** A reader copying out of a frame runs concurrently with a fill writing into it. The seqlock decides whether the bytes the reader got are *usable*, but two plain `memcpy`s racing is a data race and therefore UB whatever the version check later proves. Relaxed 64-bit atomic loads and stores are well-defined and compile to plain `mov`s on x86; the copy is head / whole words / tail so the body is one load and one 8-byte `memcpy` per word. ThreadSanitizer flagged the `memcpy` pair; this is the fix, and it costs about 50 ns on a 512-byte value against a vectorised `memcpy`.
...
`DataFile`'s contract is unchanged. `read_value` copies into `out`; `read_entry` / `read_entry_unverified` return spans into the caller's `io_buf`, exactly as `ReadOnlyPosixDataFile` does — **never into a frame**. A frame is reused memory, and an `EntryIterator` holds its span across the user's whole loop body, which can itself issue a read that evicts that very frame.

**A hit** takes no lock. Probe the index from `hash(key)` with linear probing: load the slot's version (acquire), then its key; an empty key ends the probe as a miss. On a match with an even version, load the frame number, copy the frame's overlap with the request out of the arena, fence, and reload the version. Unchanged means the bytes are exactly what that slot mapped to; otherwise retry from the top, and after eight retries fall back to a device read, which is always correct. The only write on a hit is a test-then-set of the reference bit, so a hot frame writes nothing after its first touch and 32 readers of one frame never take its cache line exclusive.
...
Every change to the index or to a frame's contents happens under `mu_`, and every change to a slot is bracketed by its version going odd and then even. **That is the whole invariant**: a reader that copied a frame while its slot's version stayed even and unchanged copied exactly the bytes that slot mapped to. Three operations:

- **`insert(key, frame, bytes)`** finds the first empty slot on the key's probe chain, marks it odd, stores frame and key, writes the frame's words, and marks it even. The slot stays odd for the whole frame write, so a reader probing for this key waits instead of copying a half-written frame.
...
- **`clock_victim()`** walks the index, clearing reference bits, until it reaches an unreferenced entry that does not belong to the active file. The hand is touched only by a thread that has already missed and needs a frame, so at ~100 ns against a 10–100 µs device read its contention is noise.
...
The seqlock writer stores the odd version and then a release fence, so the changes that follow cannot become visible before the odd version does; the even version is a release store, so a reader that sees it sees the finished bytes. The reader's acquire fence between its copy and its second version load closes the other side.
...
**MariaDB plugin, with and without a memory limit.** sysbench on a 10 M-row `sbtest1` (3.2 GB of data files, 20 M keys, ~1.1 GB of key directory), 20 s warm-up then 40 s measured, default `special` (hot-spot) distribution, `verify_checksums=OFF`, sync commits. The limited rows run `mariadbd` in a cgroup with `memory.max = 2.5 GiB`, so the page cache has about 1.2 GB for 3.2 GB of files; the pool is 512 MiB there and 4 GiB (everything resident) without a limit. Page cache dropped before every start. InnoDB is the reference with the same limit and a 1.5 GiB buffer pool (`O_DIRECT`).
