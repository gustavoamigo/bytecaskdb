...
| ID | Title | Note |
| --- | --- | --- |
| BC-050 | Add RocksDB as benchmark baseline | Added `RdbAdapter` to `benchmarks/engine_bench.cpp` and `add_requires("rocksdb", {optional = true})` to `xmake.lua`. Benchmarks: Put/Get/Del/Range50/Mixed/MixedBatch/PutMT/MixedMT matching the full LevelDB suite, at both NoSync and Sync durability levels. |
