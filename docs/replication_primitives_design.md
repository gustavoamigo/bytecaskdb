...
Vacuum does remove history: dead Puts, whole files of them, and the tombstones recovery found no longer hide a Put in another file. A stream resumed below such a vacuum would miss those entries, and a follower that already holds a Put would never learn of its dropped delete. So the engine records how far back the history is complete: `min_resumable_sequence()`, the highest sequence vacuum has dropped. Every entry above it is still on disk. `changes_since` refuses a `from_sequence` below it with `DbInvalidSequence`, and the follower re-bootstraps from a manifest (Phase 1). A follower at or above it resumes as usual (#168).

```cpp
auto changes_since(const Snapshot& snap, uint64_t from_sequence) const -> ChangeIterator;
auto min_resumable_sequence() const -> uint64_t;  // lowest from_sequence accepted
```

The bound is the snapshot's: a snapshot taken before a vacuum still pins the files it dropped, and streams from where it could before. It never decreases, and it survives a restart in the `MIN_RESUMABLE_SEQUENCE` file (see [`file_format.md`](file_format.md)). Vacuum makes the new value durable before it publishes or unlinks anything, so a crash can leave it too high, which costs a follower a re-bootstrap, but never too low. A directory without a readable record (created before the record existed, assembled from manifest files, or with the record damaged), or one a lenient open recovered with a file left out, gets the highest recovered sequence: nothing below it is known to be complete.

When the iterator is exhausted, all committed durable entries above `from_sequence` up to `min(snap.sequence(), durable_sequence)` have been delivered in order.
...
9. **Batch atomicity through vacuum**: for the `vacuumed_batches` state shape, `changes_since` over vacuumed files yields `BulkBegin`/`BulkEnd` markers framing the batch entries. The stream starts at the leader's `min_resumable_sequence()`, with the follower seeded below it from the history captured before the vacuum. `ingest` writes the batch as a single `writev` on the follower. After a simulated crash mid-ingest (Class I_C), recovery discards the incomplete batch — no partial batch is visible.
