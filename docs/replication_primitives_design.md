...
Vacuum does remove history: dead Puts, whole files of them, and the tombstones recovery found no longer hide a Put in another file. A follower resuming from a sequence older than what vacuum removed would receive a stream with those entries missing, and for a dropped tombstone, a follower that already holds the Put would never learn of the delete (#168).

So vacuum has to be coordinated with replication, and the replication service decides what it may drop: it passes `VacuumOptions::retain_after`, the lowest position a follower it counts on could resume from, and vacuum keeps every entry above it. The engine keeps no state for this. The rules for the service:

- **Every node.** Pass it on the leader and on every follower, since a follower can become leader and its history is then what the others stream.
- **Bootstraps count from the manifest.** A node being bootstrapped resumes at the manifest's `through_sequence`; count it from there as soon as the manifest is taken, before the copy finishes.
- **Leaving a follower out is a decision.** A follower the service stops counting (lost, too slow) may fall below what vacuum kept, and has to be re-bootstrapped rather than resumed. The cost of counting a stuck follower is disk: dead entries above its position stay until it moves or is dropped.
- **No replication:** the default, `kNoRetention`, restricts nothing.
...
9. **Batch atomicity through vacuum**: for the `vacuumed_batches` state shape, `changes_since` over vacuumed files yields `BulkBegin`/`BulkEnd` markers framing the batch entries. The follower holds the history up to a point, the leader vacuums with `retain_after` at that point, and the stream starts there. `ingest` writes the batch as a single `writev` on the follower. After a simulated crash mid-ingest (Class I_C), recovery discards the incomplete batch — no partial batch is visible.
